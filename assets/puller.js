...
var makeRequest = function makeRequest() {
  return new Promise(function (resolve, reject) {
    ...
    xhr.open('GET', url + ($params ? "?".concat($params) : ''));
...
var makeMessageRequest = function makeMessageRequest(name, data) {
  return new Promise(function (resolve, reject) {
    var messageXhr = new XMLHttpRequest();
    var $params = Object.keys(queryState).map(function (k) {
      return "".concat(k, "=").concat(encodeURIComponent(queryState[k]));
    }).join('&');
    messageXhr.open('POST', messageUrl + "/".concat(name) + ($params ? "?".concat($params) : ''));
    ...
    messageXhr.onload = function () {
      if (this.status >= 200 && this.status < 300) {
        ...
        reject({
          status: this.status,
          statusText: messageXhr.statusText
        });
      }
    };
...
var applyGlobalAnswer = function applyGlobalAnswer(responseResult) {
  var result = String(responseResult).trim();
  var resultJson = result ? JSON.parse(result) : null;
  applyJsonResponse(resultJson);
};

var applyJsonResponse = function applyJsonResponse(resultJson) {
      ...
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default().wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              _context.next = 4;
              return makeMessageRequest(name, data).then(function (result) {
                applyGlobalAnswer(result);
              });
